...
import { postStatus, getStatus, likePost, addComment, deleteComment, listenForComments } from '../../api/FirestoreAPI';
...
export const getComments = createAsyncThunk(
  ...
  async ({ postID }, { rejectWithValue }) => {
    try {
      // Note: listenForComments uses onSnapshot which is real-time
      // This thunk is mainly for consistency, actual comments are set via setComments action
      return { postID };
    } catch (error) {
      return rejectWithValue(error.message);
    }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(getComments.fulfilled, (state, action) => {
        ...
        // Comments will be set via setComments reducer when listener fires
      })
...
export const { setPosts, setComments, clearError } = postsSlice.actions;
